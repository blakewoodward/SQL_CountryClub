...
SELECT FB.name, M.firstname, FB.cost
FROM (SELECT F.name, B.memid, CASE WHEN B.memid = 0 THEN F.guestcost * B.slots
                              ELSE F.membercost * B.slots END AS cost
      FROM Bookings AS B
      INNER JOIN Facilities AS F
      USING(facid)
      WHERE B.starttime BETWEEN '2012-09-14' AND '2012-09-15') AS FB
INNER JOIN Members AS M
USING(memid)
ORDER BY cost DESC;
...
SELECT FB.name, FB.cost, CONCAT(M.firstname, ' ', M.surname) AS Member_Name
FROM (SELECT F.name, B.starttime, B.memid, CASE WHEN B.memid = 0 THEN F.guestcost * B.slots
                              			   ELSE F.membercost * B.slots END AS cost
...
INNER JOIN Members as M
USING(memid)
WHERE YEAR(starttime) = 2012 AND MONTH(starttime) = 9 AND DAY(starttime) = 14 AND cost > 30
ORDER BY cost DESC;
